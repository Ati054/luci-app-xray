...
echo "=== R9 apk help exit-status compatibility ==="
...
case "$*" in
    ...
    "info --help")
        command_name="info"
        help_rc="${R9_FAKE_INFO_HELP_RC:-0}"
        ;;
    "add --help")
        command_name="add"
        help_rc="${R9_FAKE_ADD_HELP_RC:-0}"
        ;;
    "adbdump --help")
        command_name="adbdump"
        help_rc="${R9_FAKE_ADBDUMP_HELP_RC:-0}"
        ;;
    "list --help")
        command_name="list"
        help_rc="${R9_FAKE_LIST_HELP_RC:-0}"
        ;;
...
if [ "${R9_FAKE_EMPTY_HELP:-}" != "${command_name}" ]; then
    case "${command_name}" in
        ...
        info)
            ...
            [ "${R9_FAKE_MISSING_CAPABILITY:-}" = "exists" ] || printf '%s\n' '  --exists'
            ;;
        add)
        ...
        adbdump)
            ...
            [ "${R9_FAKE_MISSING_CAPABILITY:-}" = "format" ] || printf '%s\n' '  --format FORMAT'
            ;;
        list)
            printf '%s\n' 'Usage: apk list [<OPTIONS>]'
            [ "${R9_FAKE_MISSING_CAPABILITY:-}" = "installed" ] || printf '%s\n' '  --installed'
            ;;
    esac
fi
exit "${help_rc}"
...
if R9_FAKE_GLOBAL_HELP_RC=1 \
    R9_PACKAGE_MARKER="${TMP_DIR}/legacy-package-operation" \
    PATH="${BIN_DIR}" \
...
sed -n '/^# BEGIN R9_APK_HELP_COMPATIBILITY_HELPERS$/,/^# END R9_APK_HELP_COMPATIBILITY_HELPERS$/p' \
    "${INSTALLER}" > "${HELPERS}"
sed -n '/^# BEGIN R9_APK_HELP_COMPATIBILITY_PREFLIGHT$/,/^# END R9_APK_HELP_COMPATIBILITY_PREFLIGHT$/p' \
    "${INSTALLER}" > "${PREFLIGHT}"

grep -q '^r9_capture_apk_help()' "${HELPERS}" && [ -s "${PREFLIGHT}" ] || {
...
set -eu
. "${R9_HELPERS}"
BACKUP_DIR="${R9_CASE_DIR}/preflight"
. "${R9_PREFLIGHT}"
[ ! -e "${R9_PACKAGE_MARKER}" ] || {
...
run_case() (
    ...
    export R9_HELPERS="${HELPERS}"
    export R9_PREFLIGHT="${PREFLIGHT}"
    export R9_CASE_DIR="${case_dir}"
    export R9_PACKAGE_MARKER="${case_dir}/package-operation"
    export PATH="${BIN_DIR}"
...
run_case subcommand_status_1 0 APK_HELP_PREFLIGHT_REACHED_END \
    R9_FAKE_INFO_HELP_RC=1 R9_FAKE_ADD_HELP_RC=1 \
