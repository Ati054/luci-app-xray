...
echo "=== R9 target portability without stat ==="
...
assert_no_target_stat "${INSTALLER}" "install_and_smoke_r9.sh"
...
sed -n '/^# BEGIN R9_TARGET_PORTABILITY_HELPERS$/,/^# END R9_TARGET_PORTABILITY_HELPERS$/p' \
    "${INSTALLER}" "${HARDWARE_SMOKE}" > "${HELPERS}"

for helper_name in r9_file_size_bytes r9_verify_exact_file_size \
    r9_exact_private_directory r9_exact_private_regular_file; do
...
cat > "${BIN_DIR}/wc" <<EOF
#!/bin/sh
case "\${R9_TEST_WC_MODE:-normal}" in
    fail) exit 1 ;;
    malformed) printf '%s\n' not-a-number; exit 0 ;;
esac
exec "${REAL_WC}" "\$@"
...
shift
for mode_path in "$@"; do
    printf '%s|%s\n' "${mode}" "${mode_path}" >> "${R9_TEST_MODE_DB}"
done
EOF
...
[ "$#" -eq 2 ] && [ "$1" = "-ld" ] || exit 2
mode_path="$2"
if [ "${R9_TEST_LS_MODE:-normal}" = "malformed" ]; then
...
chmod 0700 "${R9_PRIVATE_DIR}"
chmod 0600 "${R9_PRIVATE_FILE}"

r9_verify_exact_file_size "${R9_APK_FILE}" 6 || {
    echo "FAIL: correct APK byte size was rejected" >&2
    exit 1
}
echo "PASS: correct APK byte size is accepted without stat."

if r9_verify_exact_file_size "${R9_APK_FILE}" 7; then
    echo "FAIL: incorrect APK byte size was accepted" >&2
    exit 1
fi
echo "PASS: incorrect APK byte size is rejected."

if r9_verify_exact_file_size "${R9_MISSING_APK}" 0; then
    echo "FAIL: missing APK was accepted" >&2
    exit 1
fi
echo "PASS: missing APK is rejected."

R9_TEST_WC_MODE=fail
export R9_TEST_WC_MODE
if r9_verify_exact_file_size "${R9_UNREADABLE_APK}" 7; then
    echo "FAIL: unreadable APK was accepted" >&2
    exit 1
fi
unset R9_TEST_WC_MODE
echo "PASS: unreadable APK is rejected."

R9_TEST_WC_MODE=malformed
export R9_TEST_WC_MODE
if r9_file_size_bytes "${R9_APK_FILE}" >/dev/null; then
...
r9_exact_private_directory "${R9_PRIVATE_DIR}" || {
    echo "FAIL: directory mode 0700 was rejected" >&2
    exit 1
}
echo "PASS: real directory mode 0700 is accepted."

chmod 0755 "${R9_PRIVATE_DIR}"
if r9_exact_private_directory "${R9_PRIVATE_DIR}"; then
...
if r9_exact_private_directory "${R9_PRIVATE_DIR_LINK}"; then
    echo "FAIL: directory symlink was accepted" >&2
    exit 1
fi
echo "PASS: directory symlink is rejected."

r9_exact_private_regular_file "${R9_PRIVATE_FILE}" || {
    echo "FAIL: regular file mode 0600 was rejected" >&2
    exit 1
}
echo "PASS: real regular file mode 0600 is accepted."

chmod 0644 "${R9_PRIVATE_FILE}"
if r9_exact_private_regular_file "${R9_PRIVATE_FILE}"; then
...
if r9_exact_private_regular_file "${R9_PRIVATE_FILE_LINK}"; then
    echo "FAIL: file symlink was accepted" >&2
    exit 1
fi
echo "PASS: file symlink is rejected."

R9_TEST_LS_MODE=malformed
export R9_TEST_LS_MODE
if r9_exact_private_directory "${R9_PRIVATE_DIR}" || \
    r9_exact_private_regular_file "${R9_PRIVATE_FILE}"; then
    echo "FAIL: malformed permission output was accepted" >&2
    exit 1
fi
unset R9_TEST_LS_MODE
...
R9_HELPERS="${HELPERS}" \
R9_APK_FILE="${APK_FILE}" \
R9_UNREADABLE_APK="${UNREADABLE_APK}" \
R9_MISSING_APK="${MISSING_APK}" \
R9_PRIVATE_DIR="${PRIVATE_DIR}" \
R9_PRIVATE_DIR_LINK="${TMP_DIR}/profiles-link" \
R9_PRIVATE_FILE="${PRIVATE_FILE}" \
R9_PRIVATE_FILE_LINK="${TMP_DIR}/profile-link.json" \
R9_PACKAGE_MARKER="${PACKAGE_MARKER}" \
R9_TEST_MODE_DB="${MODE_DB}" \
PATH="${BIN_DIR}" \
/bin/sh "${HARNESS}"
